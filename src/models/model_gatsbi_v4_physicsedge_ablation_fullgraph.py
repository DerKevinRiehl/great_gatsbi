...
class GATLayerNoEdgeFeatures(nn.Module):
    def __init__(self, in_features, out_features, dropout=0.1, alpha=0.2):
        super(GATLayerNoEdgeFeatures, self).__init__()
        self.W = nn.Linear(in_features, out_features, bias=False)
        self.a = nn.Linear(2 * out_features, 1, bias=False)
        self.leakyrelu = nn.LeakyReLU(alpha)
        self.dropout = nn.Dropout(dropout)
        
    def forward(self, node_features, adj_mask=None):
        """
        node_features: [B, N, F] - node features
        adj_mask: [B, N, N] - binary adjacency mask (optional, for masking non-existent edges)
        """
        ...
        a_input = torch.cat([Wh_i, Wh_j], dim=-1)  # [B, N, N, 2 * F_out]
        e = self.leakyrelu(self.a(a_input).squeeze(-1))  # [B, N, N]

        # Mask non-existing edges (if provided)
...
class GATSBIv4_AblPhyEdg(nn.Module):
    def __init__(self, input_dim=2, hidden_dim=64, gat_out_dim=64, output_dim=2, prediction_length=25, gmm=False, num_modes=5):
        ...
        self.agent_encoder = nn.LSTM(input_dim, hidden_dim, batch_first=True)
            # GAT
        self.gat = GATLayerNoEdgeFeatures(2*hidden_dim, gat_out_dim)
            # fusion
    ...
    def forward(self, t_ego_hist, t_neighbor_hist, s_adj, t_pred_cv, t_pred_ca, t_pred_bk, t_pred_xk):
        ...
        edge_features = s_adj
        h_gat, neighbor_attention = self.gat(node_features)  # [B, N+1, gat_out_dim]        
        ...
        h_context_repeated = h_context.unsqueeze(1).repeat(1, self.prediction_length, 1)  # [batch, T_pred, gat_out_dim]  
