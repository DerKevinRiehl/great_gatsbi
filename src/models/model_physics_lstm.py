...
class PhysicsLSTM(nn.Module):
    ...
    def forward(self, ego_hist, pred_cv, pred_ca, pred_bk, pred_xk):
        ...
        # Encode constant velocity predictions (you could also just take CV as raw input instead of encoding it)
        _, (h_cv, _) = self.cv_encoder(pred_cv)
        h_cv = h_cv[-1]  # [batch, hidden_dim]

        # Encode constant acceleration predictions (you could also just take CV as raw input instead of encoding it)
        _, (h_ca, _) = self.ca_encoder(pred_ca)
        h_ca = h_ca[-1]  # [batch, hidden_dim]

        # Encode constant bicycle kinematics predictions (you could also just take CV as raw input instead of encoding it)
        _, (h_bk, _) = self.bk_encoder(pred_bk)
        h_bk = h_bk[-1]  # [batch, hidden_dim]

        # Encode constant xkalman filter predictions (you could also just take CV as raw input instead of encoding it)
