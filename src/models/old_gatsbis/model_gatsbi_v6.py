...
class GATSBIv6(nn.Module):
    def __init__(self, input_dim=2, hidden_dim=64, gat_out_dim=64, output_dim=2, prediction_length=25, history_dt=0.04, gmm=False, num_modes=5):
        ...
        # ### NETWORK STRUCTURE
            # Decay parameters
        self.history_decay_param = nn.Parameter(torch.randn(input_dim))
        self.anticipation_decay_param = nn.Parameter(torch.randn(input_dim))
            # Encoders
    ...
    def encode_agent_histories(self, ego_hist, neighbor_hists):
        ...
        # time decay
        decay_vec = torch.arange(-T+1, 1).to(device=ego_hist.device) * self.history_dt
        decay_vec = decay_vec.unsqueeze(0).unsqueeze(-1).repeat(B, 1, self.input_dim)
        decay_vec = torch.exp(decay_vec * F.softplus(self.history_decay_param))

        # Encode ego
        _, (h_ego, _) = self.agent_encoder(ego_hist * decay_vec)  # [1, B, hidden_dim]
        h_ego = h_ego.squeeze(0)                      # [B, hidden_dim]

        # Encode neighbors
        neighbor_encodings = []
        for i in range(N):
            _, (h_neigh, _) = self.agent_encoder(neighbor_hists[:, i] * decay_vec)  # [1, B, hidden_dim]
    ...
    def encode_agent_futures(self, ego_pred, neighbor_hists):
        ...
        B, N, _, _ = neighbor_hists.shape

        # time decay
        decay_vec = torch.arange(0, self.prediction_length).to(device=ego_pred.device) * self.history_dt
        decay_vec = decay_vec.unsqueeze(0).unsqueeze(-1).repeat(B, 1, self.input_dim)
        decay_vec = torch.exp(decay_vec * -F.softplus(self.anticipation_decay_param))

        _, (h_ego, _) = self.pred_encoder(ego_pred * decay_vec)
        h_ego = h_ego.squeeze(0)  # [B, hidden_dim]

        neighbor_preds = constant_velocity_predictor(neighbor_hists, prediction_length=self.prediction_length)
        neighbor_encodings = []
        for i in range(N):
            _, (h_neigh, _) = self.pred_encoder(neighbor_preds[:, i] * decay_vec)  # [1, B, hidden_dim]
            neighbor_encodings.append(h_neigh.squeeze(0))
        neighbor_encodings = torch.stack(neighbor_encodings, dim=1)  # [B, N, hidden_dim]
